...
<div class="min-h-screen flex">
    {{-- Sidebar --}}
    <aside class="w-64 text-white flex-shrink-0 border-r border-white/[0.06]" style="background-color:#2c3444">
        ...
        <nav class="py-3">
            ...
            <div x-data="{ open: {{ $assistOn ? 'true' : 'false' }} }">
                <button @click="open = !open" class="{{ $lnk($assistOn) }}" style="width:100%">
                    ...
                    <svg :style="'width:12px;height:12px;flex-shrink:0;transition:transform .2s;transform:'+( open ? 'rotate(180deg)' : 'none')" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>
                </button>
