            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                    <div class="p-6">
                        ...
                        <form method="POST" action="{{ route('admin.forms.settings.update') }}" class="mt-4 space-y-4">
                            @csrf
                            @method('PUT')

                            <div>
                                <label class="block text-xs font-semibold text-gray-600">Default recipient email(s)</label>
                                <input type="text" name="forms_default_to" value="{{ old('forms_default_to', $formsDefaultTo) }}" class="mt-1 w-full rounded-md border-gray-300" placeholder="you@example.com, admin@example.com">
                            ...
                            <div>
                                ...
                                <select name="forms_mail_mode" class="mt-1 w-full rounded-md border-gray-300">
                                    <option value="inherit" @selected(old('forms_mail_mode', $mailMode) === 'inherit')>Inherit app mailer (MAIL_*)</option>
                                    <option value="smtp" @selected(old('forms_mail_mode', $mailMode) === 'smtp')>Use Forms SMTP settings (below)</option>
                                    <option value="log" @selected(old('forms_mail_mode', $mailMode) === 'log')>Log only (do not send)</option>
                                </select>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="forms_from_name" value="{{ old('forms_from_name', $fromName) }}" class="mt-1 w-full rounded-md border-gray-300" placeholder="ImpartCMS">
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-gray-600">From email</label>
                                    <input type="email" name="forms_from_email" value="{{ old('forms_from_email', $fromEmail) }}" class="mt-1 w-full rounded-md border-gray-300" placeholder="no-reply@yourdomain.com">
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="forms_smtp_host" value="{{ old('forms_smtp_host', $smtpHost) }}" class="mt-1 w-full rounded-md border-gray-300" placeholder="smtp-relay.brevo.com">
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-gray-600">Port</label>
                                    <input type="number" name="forms_smtp_port" value="{{ old('forms_smtp_port', $smtpPort) }}" class="mt-1 w-full rounded-md border-gray-300" placeholder="587">
                                </div>
                                <div>
                                    ...
                                    <select name="forms_smtp_encryption" class="mt-1 w-full rounded-md border-gray-300">
                                        <option value="tls" @selected(old('forms_smtp_encryption', $smtpEncryption) === 'tls')>TLS</option>
                                        <option value="ssl" @selected(old('forms_smtp_encryption', $smtpEncryption) === 'ssl')>SSL</option>
                                        <option value="none" @selected(old('forms_smtp_encryption', $smtpEncryption) === '')>None</option>
                                    </select>
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-gray-600">Username</label>
                                    <input type="text" name="forms_smtp_username" value="{{ old('forms_smtp_username', $smtpUsername) }}" class="mt-1 w-full rounded-md border-gray-300">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-xs font-semibold text-gray-600">Password {{ $smtpPasswordSet ? '(already set)' : '' }}</label>
                ...
                <div class="space-y-6">
                    <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                        <div class="p-6">
                            ...
                            <form method="POST" action="{{ route('admin.forms.settings.testEmail') }}" class="mt-4 flex gap-2">
                                @csrf
                                <input type="email" name="to" value="{{ old('to') }}" class="flex-1 rounded-md border-gray-300" placeholder="you@example.com" required>
                            ...
                            <div class="mt-4 overflow-x-auto">
                                <table class="min-w-full text-sm">
                                    ...
                                    <tbody class="divide-y">
                                        @forelse($recent as $s)
                                            <tr>
                                                <td class="py-2 text-gray-700">{{ optional($s->created_at)->format('Y-m-d H:i') }}</td>
                                                <td class="py-2 text-gray-900 font-semibold">{{ strtoupper($s->mail_status) }}</td>
                                                <td class="py-2 text-gray-700">{{ $s->to_email ?: '—' }}</td>
                                                <td class="py-2 text-gray-700">{{ $s->spam_reason ?: '—' }}</td>
                                            </tr>
                                        @empty
