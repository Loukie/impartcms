...
<x-admin-layout>
        ...
        <div class="flex items-center justify-between">
            ...
            @if(!$isCreate)
                <div class="flex items-center gap-2">
                    ...
                    <form method="POST" action="{{ route('admin.forms.destroy', $form) }}" onsubmit="return confirm('Delete this form? This will also delete its submissions.');">
                        @csrf
                        @method('DELETE')
    ...
    <div class="py-8" x-data="formBuilder()" x-init="init()" x-effect="sync()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ $isCreate ? route('admin.forms.store') : route('admin.forms.update', $form) }}" @submit="sync()">
                @csrf
                @if(!$isCreate)
                    @method('PUT')
                @endif
                        ...
                        <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                            <div class="p-6 space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="slug" value="{{ old('slug', $form->slug) }}" class="mt-1 w-full rounded-md border-gray-300" placeholder="contact-us" required>
                                    <div class="mt-1 text-xs text-gray-500">Used in shortcode: <code>[form slug="…"]</code></div>
                                </div>
                                <div class="flex items-center gap-2">
                                    <input id="is_active" type="checkbox" name="is_active" value="1" class="rounded border-gray-300" @checked(old('is_active', $form->is_active))>
                                ...
                                @if(!$isCreate)
                                    <div class="text-xs text-gray-600">
                                        Shortcode:
                                        <div class="mt-1">
                                            <code class="px-2 py-1 rounded bg-gray-100">[form slug="{{ $form->slug }}" to="you@example.com"]</code>
                                        </div>
                                    </div>
                                @endif
                            </div>
        ...
        function formBuilder() {
            return {
                ...
                settings: {
                    ...
                    default_recipients: @json($defaultsCsv),
                    ui_template: @json($uiTemplate),
                    ui_columns: @json($uiColumns),
                },
                fieldsJson: '',
                settingsJson: '',
                ...
                pickMedia(fieldIdx, optIdx) {
                    ...
                    window.ImpartMediaPicker && window.ImpartMediaPicker.open({
                        url: "{{ route('admin.media.picker') }}?tab=images",
                        onSelect: (payload) => {
