<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Custom code — Trash</h2>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-3 py-2 whitespace-nowrap text-right">
                                            <div class="flex items-center justify-end gap-4">
                                                <form method="POST" action="{{ route('admin.snippets.restore', $snippet) }}" class="inline">
                                                    @csrf
                                                    <button type="submit" class="text-gray-700 hover:text-gray-900 font-semibold text-sm">Restore</button>
                                                </form>

                                                <form method="POST" action="{{ route('admin.snippets.forceDestroy', $snippet) }}"
                                                      onsubmit="return confirm('Delete permanently? This cannot be undone.');"
                                                      class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-600 hover:text-red-800 font-semibold text-sm">Delete permanently</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
