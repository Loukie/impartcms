...
class AiPageGenerator
{
    ...
    private function buildInstructions(string $styleMode, bool $fullDocument, array $designSystem = [], string $businessContext = ''): string
    {
        ...
        $rules = [
            ...
            'Output must pass this test: "Would a design director approve this for a premium brand?" If no, revise.',
            '',
            ...
            '- Background alternation: vary section backgrounds with purpose using the design system colors (var(--color-bg), light tints, and dark contrasts). Do not use the same background on every section.',
            ...
            '- Do NOT use compound or descendant CSS selectors (e.g. .parent .child { }, .section > .item { }). Every CSS rule must target a single class or element only (e.g. .split-image { }). This is required so the visual editor can read and edit each rule.',
